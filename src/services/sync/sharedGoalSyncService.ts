...
export async function syncSharedGoal(
  ...
  try {
    ...
    // 2. Prepare Bulk Operations for Goal Collection
    const goalBulkOps = linkedGoals.map((goal) => {
      ...
      if (updatedAchievementValue !== undefined) {
        updateSet.currentAchievement = updatedAchievementValue;
      }
    ...
    // 3. Prepare Bulk Operations for CheckIn Collection
    const checkinBulkOps = linkedGoals.map((goal) => {
      ...
      if (updatedAchievementValue !== undefined) {
        updateSet.actualAchievementValue = updatedAchievementValue;
      }
      if (updatedAchievementDate !== undefined) {
        updateSet.actualAchievementDate = updatedAchievementDate;
      }
    ...
    // 4. Execute batch writes in parallel
    await Promise.all([
      Goal.bulkWrite(goalBulkOps),
      CheckIn.bulkWrite(checkinBulkOps),
    ]);

    // 5. Log bulk sync activity
    ...
    console.error(
      "[SharedGoalSyncService] Error synchronizing shared goals:",
      error
    );
    throw error;
  }
